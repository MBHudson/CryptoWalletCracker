...
the script with the <i><b> "cryptowalletcracker" </b></i> command. Using a tool such at <b><i>"htop"</i></b> is strongly advised because you can see the current state of CPU usage and whether or not the script is running properly. (To install "sudo apt install htop") You can run Htop anytime in a seperate terminal with the "htop" command.
